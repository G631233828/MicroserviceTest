...
public class ResourceServiceImpl extends GeneralServiceImpl<Resource> implements ResourceService{
	 ...
	 * @throws Exception
	 */
	public List<Resource> findlistResource(String userId) {
	 ...
	 * @throws Exception
	 */
	public Resource findResourceByResUrl(String url) {
		Query query = new Query();
	...
	@Override
	public Pagination<Resource> list(Integer pageNo, Integer pageSize) {

		// 分页查询数据
		Pagination<Resource> pagination = null;
		try {
			pagination = this.findPaginationByQuery(new Query(), pageNo, pageSize, Resource.class);
			if (pagination == null)
	...
	public BasicDataResult remove(String id) {
		try {
			...
			for (String delids : strids) {
				...
				Resource rm = this.findOneById(delids, Resource.class);
