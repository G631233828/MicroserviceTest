...
public class RoleServiceImpl extends GeneralServiceImpl<Role> implements RoleService {
	...
	@Override
	public Pagination<Role> list(Integer pageNo, Integer pageSize) {
		// 分页查询数据
				Pagination<Role> pagination = null;
				try {
					pagination = this.findPaginationByQuery(new Query(), pageNo, pageSize, Role.class);
					if (pagination == null)
	...
	public BasicDataResult remove(String id) {
		try {
			...
			for (String delids : strids) {
				...
				Role rm = this.findOneById(delids, Role.class);
